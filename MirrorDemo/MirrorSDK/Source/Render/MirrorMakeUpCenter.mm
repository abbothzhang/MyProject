    ...
    do {
        ...
        if (strongData) {
            runSynchronousOnContextQueue(^{
                do {
                    p_cosmetic_engine = CCosmeticEngine::GetInstanse();
                    if (p_cosmetic_engine == NULL) {
                    ...
                    int result = p_cosmetic_engine->Initialize(_bufferSize.width, _bufferSize.height, (unsigned char *)[strongData bytes], (int)[strongData length]);
...
- (void)initializerGlass:(NSData *)fileData completed:(initialCompletedBlock)completedBlock {
    [_dataLock lock];
    do {
        if (_initializedGlass) {
            break;
        }
        ...
        runSynchronousOnContextQueue(^{
            [_dataLock lock];
            do {
                p_glass_engine = CGlassFittingEngine::GetInstance();
                if (p_glass_engine == NULL) {
                    completedBlock(_initializedGlass, [NSError errorWithDomain:@"out of memory" code:kMirrorMakeUpOutOfMemoryErrorType userInfo:nil]);
                    break;
                }
//                int result = p_glass_engine->Initialize((unsigned char *)[weakSelf.fileData bytes], (int)[weakSelf.fileData length], _bufferSize.width, _bufferSize.height, 1);
//
                int result = p_glass_engine->Initialize((unsigned char *)[weakSelf.fileData bytes], (int)[weakSelf.fileData length], _bufferSize.width, _bufferSize.height, 512, 1);

//                assert(result == kMirrorMakeUpSuccess);
                if (result == kMirrorMakeUpSuccess) {
                    _initializedGlass = YES;
                    if (!p_glass_param) {
                        p_glass_param = (p_glass_param_t)malloc(sizeof(glass_param_t));
                        memset(p_glass_param, 0, sizeof(glass_param_t));
                    }
                    completedBlock(_initializedGlass, nil);
                    break;
                }
                else {
                    completedBlock(_initializedGlass, [NSError errorWithDomain:@"initial failed" code:kMirrorMakeUpInitDataErrorType userInfo:nil]);
                    break;
                }
...
- (void)clear {
    ...
    if (p_cosmetic_engine) {
        p_cosmetic_engine->Uninitialize();
        CCosmeticEngine::ReleaseInstance(p_cosmetic_engine);
        p_cosmetic_engine = NULL;
    }
    _initialized = NO;
    if (p_glass_param) {
        [self clearGlass];
    ...
    if (p_glass_engine) {
        p_glass_engine->Uninitialize();
        CGlassFittingEngine::ReleaseInstance(p_glass_engine);
        p_glass_engine = NULL;
    }
    _initializedGlass = NO;
    [_dataLock unlock];
    self.fileData = nil;
}
...
- (BOOL)setMakeUpArray:(NSArray *)makeUpArray {
    ...
    do {
        if (p_cosmetic_engine == NULL || !_initialized) {
            break;
        }
        if (p_cosmetic_param) {
            [self clearCosmetic];
        ...
        for (MirrorMakeUpModel *current in makeUpArray) {
            ...
            if (current.makeUpType == MirrorMakeUpTypeGlass) {
                if (!p_glass_engine || !_initializedGlass) {
                    [self initializerGlass:self.fileData completed:^(BOOL finished, NSError *error) {
                        if (finished) {
                ...
                else if (current.fileData.length > 0) {
                    if (p_glass_param->p_data) {
                        [self clearGlass];
                    }
                    p_glass_param->p_data = (unsigned char *)malloc(current.fileData.length);
