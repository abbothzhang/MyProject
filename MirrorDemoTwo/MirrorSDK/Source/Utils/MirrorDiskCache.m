...
#define Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC  @"Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC"
#define Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC  @"Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC"
#define Mirror_USERDEFAULT_KEY_YETCACHEDKEY_DIC  @"Mirror_USERDEFAULT_KEY_YETCACHEDKEY_DIC"

@interface MirrorDiskCache ()
...
@implementation MirrorDiskCache

+ (instancetype)sharedCache
{
    static MirrorDiskCache *cache;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cache = [[MirrorDiskCache alloc] init];
    });
    return cache;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        _limitOfSize = 100 * 1024 * 1024; // 100MB
...
-(NSMutableDictionary *)dataMD5_yetCachedKeyDic{
    NSDictionary *dic = [_defaults objectForKey:Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC];
    if(dic == nil){
        if(!_dataMD5_yetCachedKeyDic){
            _dataMD5_yetCachedKeyDic = [[NSMutableDictionary alloc] initWithCapacity:10];
        }
        [_defaults setValue:_dataMD5_yetCachedKeyDic forKey:Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC];
    }else{
...
-(NSMutableDictionary *)dataMD5_RetainCountDic{
    NSDictionary *dic = [_defaults objectForKey:Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC];
    if(dic == nil){
        if(!_dataMD5_RetainCountDic){
            _dataMD5_RetainCountDic = [[NSMutableDictionary alloc] initWithCapacity:10];
        }
        [_defaults setValue:_dataMD5_RetainCountDic forKey:Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC];
    }else{
...
-(NSMutableDictionary *)key_yetCachedKeyDic{
    NSDictionary *dic = [_defaults objectForKey:Mirror_USERDEFAULT_KEY_YETCACHEDKEY_DIC];
    if(dic == nil){
        if(!_key_yetCachedKeyDic){
            _key_yetCachedKeyDic = [[NSMutableDictionary alloc] initWithCapacity:10];
        }
        [_defaults setValue:_key_yetCachedKeyDic forKey:Mirror_USERDEFAULT_KEY_YETCACHEDKEY_DIC];
    }else{
...
- (NSString *)mirrorCachePath
{
    if (_mirrorCachePath) {
        return _mirrorCachePath;
    }
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    _mirrorCachePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:Mirror_CACHE_DIRECTORY];
    return _mirrorCachePath;
}
...
- (NSString *)filePathForKey:(NSString *)key
{
    ...
    NSString *directoryPath = [self.mirrorCachePath stringByAppendingPathComponent:prefix];
...
//            [NSException raise:ISDiskCacheException format:@"%@", error];
    ...
    if (data != nil) {
        ...
        [defaults setValue:_dataMD5_RetainCountDic forKey:Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC];
    }
    dispatch_semaphore_signal(self.semaphore);
    return data;
}
...
//            [NSException raise:ISDiskCacheException format:@"%@", error];
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setValue:_dataMD5_yetCachedKeyDic forKey:Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC];
...
- (void)removeObjectsUsingBlock:(BOOL (^)(NSString *))block
{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    NSFileManager *fileManager = [NSFileManager defaultManager];
    for (NSString *path in [fileManager subpathsAtPath:self.mirrorCachePath]) {
        NSString *filePath = [self.mirrorCachePath stringByAppendingPathComponent:path];
...
- (void)removeAllObjects
{
    ...
    for (NSString *filePath in [self validFilePathsUnderPath:self.mirrorCachePath]) {
    ...
    [defaults setValue:_dataMD5_yetCachedKeyDic forKey:Mirror_USERDEFAULT_DATA_YETCACHEDKEY_DIC];
    [defaults setValue:_dataMD5_RetainCountDic forKey:Mirror_USERDEFAULT_DATA_RETAINCOUNT_DIC];
    [defaults setValue:_dataMD5_yetCachedKeyDic forKey:Mirror_USERDEFAULT_KEY_YETCACHEDKEY_DIC];
...
- (void)calculateCurrentSize
{
    ...
    [self.calculationQueue addOperationWithBlock:^{
        ...
        NSInteger sum = 0;
        for (NSString *filePath in [strongSelf validFilePathsUnderPath:strongSelf.mirrorCachePath]) {
