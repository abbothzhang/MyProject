//
//  B_DeliveryListViewController.m
//  GeneralHospital
//
...
@interface B_DeliveryListViewController ()

@end

@implementation B_DeliveryListViewController
...
-(void)selectIndex:(UISegmentedControl *)sender
{
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
        ...
        ASINetRequest* NetRequest=[ASINetRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://app.zipn.cn/app/order/list.jhtml?status=%ld&page=1",(long)sender.selectedSegmentIndex+1]]];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return 135+[[[ListArray objectAtIndex:[indexPath row]] ObjectForKey:@"itemList"] count]*100;
}
...
-(void)ShopDetailAct:(UIButton *)sender
{
    ...
    B_ShopDetailView.Dict=[ListArray objectAtIndex:sender.tag];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIView *backView=[[UIView alloc] initWithFrame:CGRectMake(0, 14, SCREEN_WIDTH, 121+100*[cellArray count])];
    ...
    nameLabel.text=[[ListArray objectAtIndex:row] ObjectForKey:@"title"];
    ...
    for(int i=0;i<[cellArray count];i++)
    {
        ...
        deleteBtn.accessibilityLanguage=[NSString stringWithFormat:@"%ld,%d",row,i];
    ...
    UILabel* titleLabel=[[UILabel alloc] initWithFrame:CGRectMake(10, 45+100*[cellArray count],50, 35)];
    ...
    UILabel* stateLabel=[[UILabel alloc] initWithFrame:CGRectMake(60, 45+100*[cellArray count],80, 35)];
    ...
    UILabel* numLabel=[[UILabel alloc] initWithFrame:CGRectMake(120, 45+100*[cellArray count],130, 35)];
    numLabel.text=[NSString stringWithFormat:@"共%d项   金额：",[cellArray count]];
    ...
    UILabel* moneyLabel=[[UILabel alloc] initWithFrame:CGRectMake(250, 45+100*[cellArray count],80, 35)];
    ...
    switch ([[[ListArray objectAtIndex:row] ObjectForKey:@"status"] intValue]) {
        case 1:
        {
            ...
            payBtn.tag=row;
            payBtn.frame = CGRectMake(SCREEN_WIDTH-170, 80+100*[cellArray count], 70, 30);
            ...
            cancelBtn.tag=row;
            cancelBtn.frame = CGRectMake(SCREEN_WIDTH-90, 80+100*[cellArray count], 70, 30);
        ...
        case 3:
        {
            ...
            logisticsBtn.tag=row;
            logisticsBtn.frame = CGRectMake(SCREEN_WIDTH-180, 80+100*[cellArray count],80, 30);
            ...
            [logisticsBtn setHidden:YES];
            
            [backView addSubview:logisticsBtn];
            [[logisticsBtn layer] setMasksToBounds:YES];
            [[logisticsBtn layer] setCornerRadius:2];
            [[logisticsBtn layer]setBorderWidth:1];
            [[logisticsBtn layer]setBorderColor:[STYLECLOLR CGColor]];
            if ([[ListArray objectAtIndex:row] objectForKey:@"deliveryURL"]==nil) {
                logisticsBtn.hidden=YES;
            }
            
            UIButton *reviewBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            reviewBtn.tag=row;
            reviewBtn.frame = CGRectMake(SCREEN_WIDTH-90, 80+100*[cellArray count], 70, 30);
        ...
        case 4:
        {
            ...
            afterSalesBtn.tag=row;
            afterSalesBtn.frame = CGRectMake(SCREEN_WIDTH-180, 80+100*[cellArray count],80, 30);
           ...
           // [backView addSubview:afterSalesBtn];
...
-(void)reviewAct:(UIButton *)sender
{
    ...
    B_CommentView.UseDict=[ListArray objectAtIndex:sender.tag];
