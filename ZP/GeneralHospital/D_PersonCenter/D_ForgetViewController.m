...
#import "D_ForgetViewController.h"
#import "BDRSACryptor.h"
#import "BDRSACryptorKeyPair.h"
#import "BDError.h"
#import "BDLog.h"
@interface D_ForgetViewController ()

@end

@implementation D_ForgetViewController
...
- (void)viewDidLoad
{
    ...
    [RegistBtn setTitle:@"确认提交" forState:UIControlStateHighlighted];
    RegistBtn.titleLabel.font = [UIFont boldSystemFontOfSize:20];
...
-(void)NextStep
{
    ...
    if ([[Password.text stringByReplacingOccurrencesOfString:@" " withString:@""] length]==0) {
        UIAlertView* alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"密码不能是空格" delegate:self cancelButtonTitle:NSLocalizedString(@"sure", nil) otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    ...
    ASINetRequest* NetRequest=[ASINetRequest requestWithURL:[NSURL URLWithString:@"http://app.zipn.cn/app/user/reset_password.jhtml"]];
