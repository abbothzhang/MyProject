...
-(BOOL)isFaceModelDownLoad{
    NSString *facelModelPath = [[NSBundle mainBundle] pathForResource:@"mirror_face_model" ofType:@"dat"];
    NSData *faceModelData = [[NSData alloc] initWithContentsOfFile:facelModelPath];
    [[MirrorDiskCache sharedCache] cacheObject:faceModelData forKey:self.cacheKeyFaceModel];
...
-(void)downLoadFaceModel{
    ...
    fileUrl = @"http://algorithm-ps.oss-cn-hangzhou.aliyuncs.com/temp/BD71C43C4E3943FC6F5B99FBFB82FE2E.dat";
    self.downloadManager = [[MirrorDownload alloc] initWithURL:[NSURL URLWithString:fileUrl]];
    self.downloadManager.key = @"downLoad1";
