...
-(NSDictionary*)parseJsonToDic:(id)data
{
    ...
    if ([data isKindOfClass:[NSString class]]) {
        ...
        NSError *err;
        paramDict = [NSJSONSerialization JSONObjectWithData:dataObj options:NSJSONReadingAllowFragments error:&err];
        if(err) {
            NSLog(@"json解析失败：%@",err);
            return nil;
        }
