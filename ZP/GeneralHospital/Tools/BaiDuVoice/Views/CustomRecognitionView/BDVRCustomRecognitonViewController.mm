    ...
    switch (aStatus)
    {
        case EVoiceRecognitionClientWorkStatusFlushData: // 连续上屏中间结果
        {
            ...
            if ([text length] > 0)
            {
                 ...
                 //[clientSampleViewController logOutToManualResut:text];
        ...
        case EVoiceRecognitionClientWorkStatusFinish: // 识别正常完成并获得结果
        {
            ...
            for(NSString *string in (NSArray *)aObj)
            {
                NSLog(@"1111=%@",string);
            }
            ...
            {
                [clientSampleViewController logOutToManualResut:[aObj firstObject]];
            }
            else
            {
...
- (void)createErrorViewWithErrorType:(int)aStatus
{
    ...
    [clientSampleViewController logOutToManualResut:nil];
}
...
- (void)timerFired:(id)sender
{
    ...
    [clientSampleViewController VoiceLevel:voiceLevel];
}
